...
| `agent.control_center` | select attach_tui/headless mode for one agent, bind session/transport, send input, observe output, pause/stop/wait/retry | implicit mode fallback rejected; tmux adapter cannot own headless truth; SDK adapter cannot own task truth; headless run cannot fall back to a per-command SDK process; normal scoped stop cannot project `error` | attach_tui tmux-bound smoke and headless SDK bridge create/run/stop/recover smoke on one agent; recovery must show same persisted `thread_id` and a new bridge PID |
| `startup.session` | init project session, init configured root manager, spawn workers, hand the live agent to Agent Control Center; judge manager output status from session health and adapter hints | manager not persistence truth; no direct tmux; session path under home; startup events persisted; silence after launch is not forced to error | `agentteam start --cwd <cwd> --config docs/config/config.toml.example --team default --json`, then control-plane binding keeps a live session `busy` until idle/fault projection |
